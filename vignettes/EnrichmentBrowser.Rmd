...
already calculated for each gene, and your are now interested
whether certain gene functions are enriched for differential
...
rather has to be read in from file. This can be done using the function
...
in *Bioconductor*, which discourage use of *ExpressionSet* in favor of
*SummarizedExperiment*. Major reasons are the compatibility of
*SummarizedExperiment* with operations on genomic regions as well as
efficient dealing with big data.
...
Thus, we consider two example data sets -- a microarray and a RNA-seq
data set, and discuss similarities and differences of the respective
analysis steps.
...
as as you have the corresponding annotation package, here the
...
The expression change of highest statistical significance is observed
...
-   SAFE: Significance Analysis of Function and Expression (resampling
    ...
    Wilcoxon's rank sum, and allows to estimate the significance of gene
...
![(#fig:oraRes) ORA result view For each significant gene set in the
...
in red.](ora_ebrowse.png){width="20cm" height="8.75cm"}
...
`r Biocpkg("EnrichmentBrowser")` package allows to combine results from
...
![(#fig:browsecomb) Combined result view By clicking on one of the
columns (ORA.RANK, ..., GGEA.PVAL) the result can be interactively
ranked according to the selected
criterion.](comb_ebrowse.png){width="20cm" height="6.8cm"}
