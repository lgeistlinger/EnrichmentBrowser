...
defined in the \Rfunction{rowData} slot.
...
Using precomputed information, normalization within and between lanes can be 
carried out via
...
This can be performed using the function \Rfunction{de.ana}
...
<<DE-ana-airway>>=
airSE <- de.ana(airSE, de.method="edgeR")
...
that are known to work together, e.g. as defined in the Gene Ontology 
...
pathways for a chosen organism (here: \emph{Homo sapiens}) as gene sets.

<<get-kegg-gs, eval=FALSE>>=
kegg.gs <- get.kegg.genesets("hsa")
@

Analogously, the function \Rfunction{get.go.genesets} retrieves GO 
...
$\alpha$ of 0.1
<<sbea>>=
sbea.res <- sbea(method="ora", eset=allSE, gs=hsa.gs, perm=0, alpha=0.1)
gs.ranking(sbea.res)
@
...
<<sbea2>>=
sbea.res2 <- sbea(method=dummy.sbea, eset=allSE, gs=hsa.gs)
...
We can download all KEGG pathways of a specified organism 
(here: \emph{Homo sapiens}) via 

<<dwnld-pwys, eval=FALSE>>=
pwys <- download.kegg.pathways("hsa")
@

For demonstration purposes, we use a selection of already downloaded human 
KEGG pathways.
...
<<nbea>>=
nbea.res <- nbea(method="ggea", eset=allSE, gs=hsa.gs, grn=hsa.grn)
gs.ranking(nbea.res)
@
...
\section{Advanced: configuration parameters}
...
changed using the function \Rfunction{config.ebrowser}.
