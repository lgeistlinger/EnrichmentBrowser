...
usage <- function()
{
	...
	message(use.message)
    message("See http://www.bioconductor.org/packages/release/bioc/manuals/EnrichmentBrowser/man/EnrichmentBrowser.pdf#page.10\n")
    quit(save="no")
}
...
{
	dirs <- list.files(out.dir, 
			pattern="_RESULT_files$", 
			full.names=TRUE)
	...
	for(d in dirs)
	{
		...
		for(f in files) 
            file.rename(from=f, to=paste(out.dir, basename(f), sep=""))
		file.remove(d)
	}
}
...
main <- function(cmd.args)
{
    ...
    data.type <- NULL
	if("--dtype" %in% names(args)) data.type <- args[["--dtype"]]

    norm.method <- "none"
	if("--nmeth" %in% names(args)) norm.method <- args[["--nmeth"]]

	de.method <- "limma"
	if("--dmeth" %in% names(args)) de.method <- args[["--dmeth"]]

	alpha <- 0.05
	if("--alpha" %in% names(args)) alpha <- as.numeric(args[["--alpha"]])

    beta <- 1
	if("--beta" %in% names(args)) alpha <- as.numeric(args[["--beta"]])

	perm <- 1000
	if("--perm" %in% names(args)) perm <- as.integer(args[["--perm"]])

	out.dir <- file.path(getwd(), "eBrowserOut")
    if(file.exists(out.dir)) system(command=paste("rm -rf", file.path(out.dir, "*")))
	if("--outd" %in% names(args))
    {
        outd <- args[["--outd"]] 
        if(dirname(outd) %in% list.files(".")) outd <- file.path(getwd(), outd)
        if(file.exists(outd)) system(command=paste("rm -rf", file.path(outd, "*")))
    }

	nr.show <- 10
    if("--show" %in% names(args)) nr.show <- as.integer(args[["--show"]])
    ...
    if(file.exists(gs)) gs <- getGenesets(gs)
    else if(gs=="GO") gs <- getGenesets(org, db="go")
    else if(gs=="KEGG") gs <- getGenesets(org, db="kegg") 
    ...
    if(is.null(data.type)) data.type <- autoDetectDataType(assay(se))
    if(is.null(meth))
    {
        message("No enrichment method selected")
        meth <- sbeaMethods()
        if(!is.null(grn)) meth <- c(meth, nbeaMethods())
        message(paste("Executing", paste(meth, collapse=",")))
    }

    ebrowser(
        meth=meth, exprs=se, org=org, #cdat=cdat, rdat=rdat,  
        data.type=data.type, norm.method=norm.method, de.method=de.method,
        gs=gs, grn=grn, perm=perm, alpha=alpha, beta=beta,
